...
while sample < total_runs:
    try:
        ...
        for train, test in kfold.split(training_data, training_labels):
            ...
            nn = Sequential()
            nn.add(Dense(8, activation='relu', input_shape=training_data[0].shape))
            nn.add(Dense(units=2, activation='softmax'))
            nn.compile(optimizer='adadelta',
                          loss='categorical_crossentropy',
                          metrics=['accuracy', categorical_accuracy])
            nn.fit(training_data[train], training_one_hot_encoded_labels[train], validation_split=0.2, epochs=50, batch_size=100)
